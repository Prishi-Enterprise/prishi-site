...
const products = JSON.parse(await readFile(path.join(root, 'src/products.json'), 'utf8')).filter(p => p.approved === true);
if (!products.length) throw new Error('At least one owner-approved product is required.');
for (const product of products) {
  const url = new URL(product.url);
  if (url.protocol !== 'https:') throw new Error('Product URLs must use HTTPS.');
  if (url.hostname !== product.domain) throw new Error('Product domain must match its URL.');
}
const cards = products.map(p => `<article class="product-card" aria-labelledby="${escape(p.id)}-title">
...
const html = template.replace('{{PRODUCTS}}', cards).replace('{{PRODUCT_COUNT}}', `${String(products.length).padStart(2, '0')} ${products.length === 1 ? 'product' : 'products'} live`).replace('{{YEAR}}', String(new Date().getFullYear()));
...
console.log(`Built Prishi portfolio with ${products.length} approved product(s).`);
